...
public class AutoRoutes extends Module implements Accessor {
  ...
  @Getter
  private static final ColourSetting useColour = new ColourSetting("Use", Colour.WHITE.copy()); // idk

  private final SaveSetting<HashMap<String, List<Node>>> data = new SaveSetting<>("Nodes", "routes", "routes.json", HashMap::new, new TypeToken<HashMap<String, List<Node>>>() {
  }.getType(), new GsonBuilder().registerTypeHierarchyAdapter(Node.class, new NodeAdapter()).setPrettyPrinting().create(), true, this::reload, null);
  ...
  private static final Set<String> SECRET_NAMES = Set.of("Health Potion VIII Splash Potion", "Healing Potion 8 Splash Potion", "Healing Potion VIII Splash Potion", "Healing VIII Splash Potion", "Healing 8 Splash Potion", "Decoy", "Inflatable Jerry", "Spirit Leap", "Trap", "Training Weights", "Defuse Kit", "Dungeon Chest Key", "Treasure Talisman", "Revive Stone", "Architect's First Draft", "Secret Dye", "Candycomb");

  public AutoRoutes() {
    this.registerProperty(editMode, centerOnly, zeroTickBreak, use1_8Height, triggerBind, triggerBindGui, addBlockBind, routeStartBind, data, render);
...
//        ChatUtils.chat("Poll Input: " + this.forceNextNotSneak);
  ...
  public List<Node> getStartNodes(UniqueRoom uniqueRoom) {
    ...
    if (activeNodes.containsKey(room.getData()))
      return activeNodes.get(room.getData()).stream().filter(Node::isStart).toList();
  ...
  public boolean clearNodes(UniqueRoom uniqueRoom) {
    if (Minecraft.getInstance().player == null || !this.activeNodes.containsKey(uniqueRoom.getMainRoom().getData()))
      return false;
  ...
  public void onReceivePacket(PacketEvent.Receive event) {
    if (!Location.getArea().is(Island.Dungeon) || Map.getCurrentRoom() == null || this.realNode == null || mc.level == null || !this.realNode.hasAwaits() || !this.realNode.getAwaitManager().hasAwait(AwaitType.SECRETS))
      return;
    ...
    if (event.getPacket() instanceof ClientboundTakeItemEntityPacket packet) {
      ...
      packet.getEntityIds().forEach(id -> {
        ...
        if (entity instanceof ItemEntity itemEntity && entity.distanceToSqr(mc.player) < 64 && SECRET_NAMES.contains(ChatFormatting.stripFormatting(itemEntity.getItem().getHoverName().getString()))) {
          this.realNode.getAwaitManager().consume(AwaitSecrets.class, 1);
        }
      });
    }
  ...
  private void routeToStart() {
    ...
    Node closestStart = this.activeNodes.get(currentRoom.getData()).stream().filter(Node::isStart).min(Comparator.comparingDouble(n -> n.getRealPos().squaredDistanceTo(startPos))).orElse(null);
