...
public class NodeAdapter implements JsonDeserializer<Node>, JsonSerializer<Node> {
  private static final Type posType = new TypeToken<Pos>() {
  }.getType();
  ...
  public Node deserialize(JsonElement json, Type typeOfT, JsonDeserializationContext context) throws JsonParseException {
    ...
    return switch (type) {
      case "ETHERWARP" ->
        new EtherwarpNode(localPos, gson.fromJson(obj.get("localTarget").getAsJsonObject(), posType), awaits, start);
      ...
      case "AOTV" ->
        new AotvNode(localPos, gson.fromJson(obj.get("rotationVec").getAsJsonObject(), posType), awaits, start);
      case "BREAK" ->
        new BreakNode(localPos, gson.fromJson(obj.getAsJsonArray("blocks"), new TypeToken<ArrayList<Pos>>() {
        }.getType()), awaits, start);
      case "USE" ->
        new UseNode(localPos, gson.fromJson(obj.get("rotationVec").getAsJsonObject(), posType), obj.get("itemID").getAsString(), obj.get("sneak").getAsBoolean(), awaits, start);
