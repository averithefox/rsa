...
public class AutoTerms extends Module {
  ...
  @Getter
  private static final MultiBoolSetting terminals = new MultiBoolSetting("Terminals", List.of("Colours", "Melody", "Numbers", "Red Green", "Rubix", "Starts With"), List.of("Colours", "Melody", "Numbers", "Red Green", "Rubix", "Starts With"));
...
//                ChatUtils.chat("new : " + newState.getHash());
  ...
  public void onPlayerTick(ClientTickEvent.Player event) {
    if (Minecraft.getInstance().player == null || Location.getArea() != Island.Dungeon || !DungeonUtils.isPositionInF7Boss(Minecraft.getInstance().player.position()) || !isInTerm() || !invWalkGroup.getValue().isEnabled())
      return;
  ...
  public void onReceivePacket(PacketEvent.Receive event) {
    ...
    if (event.getPacket() instanceof ClientboundOpenScreenPacket packet) {
      ...
      if (invWalkGroup.getValue().isEnabled()) {
        ...
        if (invWalkGroup.getValue().getInvwalkMaybeFix().getValue())
          setContainerMenu(packet.getType(), mc, packet.getContainerId(), packet.getTitle());
      }
      return;
    }
