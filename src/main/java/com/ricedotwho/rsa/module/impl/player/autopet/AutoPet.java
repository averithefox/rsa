...
public class AutoPet extends Module {
  ...
  public void onReceivePacket(PacketEvent.Receive event) {
    ...
    if (event.getPacket() instanceof ClientboundContainerSetSlotPacket packet) {
      if (packet.getContainerId() < 1 || packet.getContainerId() > 100 || mc.player == null || this.container == null || this.container.containerId != packet.getContainerId())
        return;
      event.setCancelled(true);
      ...
      if (swapping) {
        ...
        if (!ChatFormatting.stripFormatting(item.getHoverName().getString()).toLowerCase().contains(swapID) && !ItemUtils.getUUID(item).equals(swapID) && !ItemUtils.getID(item).equals(swapID))
          return;
