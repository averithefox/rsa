...
public class BonzoHelper extends Module {
  ...
  public void onPacketSend(PacketEvent.Send event) {
    if (!(event.getPacket() instanceof ServerboundUseItemPacket packet) || mc.player == null || packet.getHand() != InteractionHand.MAIN_HAND)
      return;
  ...
  public void onPacket(PacketEvent.Receive event) {
    if (!(event.getPacket() instanceof ClientboundSetEntityMotionPacket motionPacket) || mc.player == null || motionPacket.getId() != mc.player.getId() || !awaitingVelo)
      return;
    TickFreeze.unFreeze();
    awaitingVelo = false;
  }
