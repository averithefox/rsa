...
public class BreakerAura extends Module {
  ...
  private final SaveSetting<Set<Pos>> data = new SaveSetting<>("Aura Blocks", "dungeon/breaker", "breaker_aura.json", HashSet::new, new TypeToken<Set<Pos>>() {
  }.getType(), FileUtils.getPgson(), true, null, null);
...
//        }
  ...
  public void onItemUpdate(PacketEvent.PostReceive event) {
    if (!(event.getPacket() instanceof ClientboundContainerSetSlotPacket packet) || !"DUNGEONBREAKER".equals(ItemUtils.getID(packet.getItem())) || !Location.getArea().is(Island.Dungeon))
      return;
  ...
  private Optional<Pos> getClosest(Set<Pos> positions) {
    ...
    for (Pos pos : positions) {
      ...
      if (shape.isEmpty() || !DungeonBreaker.canInstantMine(state) || InteractUtils.faceDistance(vec3, mc.player.position().add(0, mc.player.getEyeHeight(mc.player.getPose()), 0)) > InteractUtils.BLOCK_RANGE)
        continue;
