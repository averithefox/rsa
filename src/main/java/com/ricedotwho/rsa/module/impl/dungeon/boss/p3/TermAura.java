...
public class TermAura extends Module {
  ...
  public void onChat(ChatEvent.Chat event) {
    if (!Location.getArea().is(Island.Dungeon) || Minecraft.getInstance().player == null || !DungeonUtils.isPositionInF7Boss(Minecraft.getInstance().player.position()))
      return;
    if (!event.getMessage().getString().startsWith("This Terminal doesn't seem to be responsive at the moment."))
      return;
  ...
  private void rapeArmorstands() {
    if (Minecraft.getInstance().player == null || Minecraft.getInstance().level == null || Minecraft.getInstance().getConnection() == null)
      return;
