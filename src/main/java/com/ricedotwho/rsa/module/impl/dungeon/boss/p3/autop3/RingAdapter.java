...
public class RingAdapter implements JsonDeserializer<Ring>, JsonSerializer<Ring> {
  ...
  public Ring deserialize(JsonElement json, Type typeOfT, JsonDeserializationContext context) throws JsonParseException {
    ...
    return switch (type) {
      ...
      case FAST_BONZO ->
        new FastBonzoRing(min, max, obj.get("yaw").getAsFloat(), obj.get("pitch").getAsFloat(), args, sub);
      ...
      case USE ->
        new UseRing(min, max, obj.get("item").getAsString(), obj.get("yaw").getAsFloat(), obj.get("pitch").getAsFloat(), args, sub);
