...
//        });
  ...
  public void onGuiOpen(GuiEvent.Loaded event) {
    if (mc.player == null || !(mc.player.containerMenu instanceof ChestMenu menu) || !running || action != Action.CROESUS || mc.screen == null || !mc.screen.getTitle().getString().equals("Croesus") || !Location.getArea().is(Island.DungeonHub))
      return;
...
//        boolean canChestKey = !modiLore.get(5).getStyle().isStrikethrough();
  ...
  public void onClose(PacketEvent.Send event) {
    if (!(event.getPacket() instanceof ServerboundContainerClosePacket) || !Location.getArea().is(Island.DungeonHub))
      return;
