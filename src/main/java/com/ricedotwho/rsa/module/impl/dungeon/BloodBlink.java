    ...
    switch (state) {
      ...
      case 0: {
        ...
        PacketOrderManager.register(PacketOrderManager.STATE.ITEM_USE, () -> {
          if (!SwapManager.swapItem(Items.DIAMOND_SHOVEL) || !player.getLastSentInput().shift() || startRoom == null)
            return;
      ...
      case 17: {
        ...
        if (((africanSlavePingMode.getValue() && ticksTilStart != -67 && ticksTilStart <= 0) || Dungeon.isStarted()) && (serverTickTimer % 40) < (40 - bloodLoadTickTime.getValue().intValue())) {
          ...
          PacketOrderManager.register(PacketOrderManager.STATE.ITEM_USE, () -> {
            ...
            if (!SwapManager.swapItem(i -> i.getItem() == Items.ENDER_PEARL && isNormalEnderpearlID(ItemUtils.getID(i))))
              return;
    ...
    if (packet instanceof ClientboundPlayerPositionPacket s08) {
      switch (state) {
        ...
        case 30: {
          ...
          if (!isInRoom(Mth.floor(pos.x()), Mth.floor(pos.z()), targetRoom) || pos.y < targetRoom.getBottom() - 1 || pos.y > targetRoom.getBottom() + 7)
            break; // 66 bedrock block blood, this stops aotv S08s from getting considered as pearls
