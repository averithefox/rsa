...
public abstract class Node {
  ...
  public boolean isInNode(Pos playerPos) {
    if (AutoRoutes.getCenterOnly().getValue()) {
      return this.realPos.x() == playerPos.x() && playerPos.y() >= this.realPos.y() - 0.05 && playerPos.y() <= this.realPos.y() + 0.05 && this.realPos.z() == playerPos.z();
    }
  ...
  public boolean updateNodeState(Pos playerPos, int tickTime) {
    if (tickTime <= lastTickTime)
      return false; // Don't go do the same node twice in 1 tick, also blocks from setting it to untriggered
