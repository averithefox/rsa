...
public class MovementRecorder extends SubModule<AutoP3> {
  ...
  private static final SaveSetting<List<PlayerInput>> data = new SaveSetting<>("Route", "dungeon/recorder", "inputs.json", ArrayList::new,
    new TypeToken<List<PlayerInput>>() {
    }.getType(),
    new GsonBuilder()
  ...
  public void onPacket(PacketEvent.Send event) {
    if (state != State.RECORDING || mc.player == null || !(event.getPacket() instanceof ServerboundUseItemPacket packet) || recorded.isEmpty())
      return;
  ...
  public static void resumeRecording() {
    ...
    if (data.getValue() == null || playIndex >= data.getValue().size() || data.getValue().get(playIndex) == null)
      return;
