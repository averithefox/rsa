...
public class Blink extends Module implements ClientRotationProvider {
  ...
  public void blinkMovement(List<MovementRecorder.PlayerInput> movements) {
    if (Minecraft.getInstance().level == null || Minecraft.getInstance().player == null || Minecraft.getInstance().getConnection() == null || packetCount < 1 || movements.isEmpty())
      return;
  ...
  public void onLocalPlayerTick(ClientTickEvent.Player event) {
    if (packetCount >= maxBlinkPacket.getValue().intValue() || smallMovementDelta.getValue().intValue() == 0 || !MovementRecorder.isIdle())
      return;

    LocalPlayer player = event.getPlayer();
    if (Minecraft.getInstance().level == null || Minecraft.getInstance().player == null || player.getId() != Minecraft.getInstance().player.getId())
      return;
